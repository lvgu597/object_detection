...
【tricks】mixup不仅是要线性插值样本X，也需要线性插值标签Y；三个或以上的样本进行mixup并不会带来更多的收益；模型容量越大或者训练时间越长minup带来的收益越多。

详细理解：https://www.zhihu.com/question/67472285/answer/256651581（原作者回答）

#### Cascade R-CNN

【地址】https://arxiv.org/abs/1712.00726

【主要贡献】找出尽量少包含接近负样本框的detector；根据输入IOU不同设置不同阈值使得检测效果最佳

【文章结构】根据图表简要介绍→相关工作→

【算法思想】

【tricks】1.训练后的detector几乎总能提升input bbox的质量；2. **单IOU detector对接近其训练IOU阈值的bbox是最优的** 3. 单纯增加训练时的IOU阈值并不能直接提高detector质量
